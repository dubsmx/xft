    ...
    <section className="py-16 md:py-24 bg-white">
      <div className="max-w-[1200px] mx-auto px-6">
        ...
        <div className="relative bg-gradient-to-r from-[#8E1979] to-[#b92b9c] rounded-[2.5rem] shadow-2xl overflow-hidden p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-10 md:gap-0 border-4 border-white ring-1 ring-gray-100">
            ...
            {/* --- LADO IZQUIERDO: ICONOS ANIMADOS --- */}
            <div className="relative w-full md:w-1/2 flex justify-center items-center h-[280px] z-10">
                ...
                {/* ICONO CENTRAL: PAQUETE DE GUÍAS (z-10) */}
                <motion.div 
                    animate={{ y: [0, -15, 0] }}
                ...
                {/* ICONO SATÉLITE 1: DESCUENTO % (SE AÑADIÓ z-20) */}
                <motion.div 
                    animate={{ y: [0, 10, 0], rotate: [0, 10, 0] }}
                    transition={{ duration: 5, repeat: Infinity, ease: "easeInOut", delay: 0.5 }}
                    className="absolute top-10 right-10 md:right-20 bg-white text-[#8E1979] p-4 rounded-full shadow-xl border-4 border-[#fce7f6] transform rotate-12 z-20"
                >
                    <Percent size={32} fill="currentColor" />
                </motion.div>

                {/* ICONO SATÉLITE 2: ETIQUETAS MÚLTIPLES (SE AÑADIÓ z-20) */}
                <motion.div 
                    animate={{ y: [0, -10, 0], x: [0, -5, 0] }}
                    transition={{ duration: 6, repeat: Infinity, ease: "easeInOut", delay: 1 }}
                    className="absolute bottom-16 left-10 md:left-20 bg-[#46075E] p-4 rounded-2xl shadow-xl border border-white/20 z-20"
                >
                    <Tags size={32} className="text-white" />
                </motion.div>

                 {/* ETIQUETA DE PRECIO FLOTANTE (SE AÑADIÓ z-20) */}
                 <motion.div 
                    animate={{ scale: [1, 1.1, 1] }}
                    transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}
                    className="absolute top-20 left-16 md:left-24 bg-[#25D366] px-3 py-1 rounded-full shadow-lg transform -rotate-6 border-2 border-white/50 z-20"
                >
        ...
        {/* ... resto del componente (botones, disclaimer) ... */}
